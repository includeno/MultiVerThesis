...
public class JsonDataGenerator {
    ...
    public static void collectAllElementsWithoutIdAndSortIndex(Element element, List<Element> result) {
        result.add(element);
        if (element.contents != null) {
            for (Element child : element.contents) {
                collectAllElementsWithoutIdAndSortIndex(child, result);
            }
        }
    }

    public static void assignIdAndSortIndex(List<Element> elements) {
        int globalId = 1;
    ...
    static {
        ...
        for (Element element : jsonData) {
            collectAllElementsWithoutIdAndSortIndex(element, allElements);
        }
        assignIdAndSortIndex(allElements);
    }
